...
class Experience(object):
    ...
    def sample_trajectories(self, batch_size, sequence_size):
        """
        ACER 采样多个 trajectories
        :param batch_size: 获取多少个 trajectory
        :param sequence_size:
        :return: [batch_size, min{sizeof trajectory}]
        """
        min_len = sequence_size
        batch = []
        for i in range(batch_size):
            batch.append(self.sample_sequence(sequence_size=sequence_size))
            min_len = min(min_len, len(batch_size[-1]))
        # 按照最小的长度裁剪
